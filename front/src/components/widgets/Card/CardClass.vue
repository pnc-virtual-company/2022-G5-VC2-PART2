<template>
  <div class="w-full flex mt-10">
    <div
      class="w-4/12 m-3 mt-10 items-center justify-center shadow rounded bg-white"
      v-for="card of classes"
      :key="card"
      @click="removeClass"
    >
      <div
        class="w-full rounded shadow flex justify-between items-center bg-warning"
      >
        <h1
          class="w-full h-full text-center justify-center rounded uppercase text-white text-2xl font-bold"
        >
          {{ card.name }}
        </h1>
        <div class="flex">
          <div
            class="flex items-center p-2 text-sm text-primary"
            @click="editClass"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              stroke-width="1.5"
              stroke="currentColor"
              class="w-5 h-5"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
                d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10"
              />
            </svg>
          </div>
          <div
            class="flex items-center p-2 mt-2 text-sm text-red-500"
            @click="alertDelete"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              stroke-width="1.5"
              stroke="currentColor"
              class="w-5 h-5"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
                d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"
              />
            </svg>
          </div>
        </div>
      </div>
      <div class="text-center text-xl p-9">
        {{ card.students.number }} Students
        <div class="flex justify-end mt-2 text-primary">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            stroke-width="1.5"
            stroke="currentColor"
            class="w-6 h-6"
          >
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
              d="M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198l.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 016 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 00-4.681 2.72 8.986 8.986 0 003.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 11-6 0 3 3 0 016 0zm6 3a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0zm-13.5 0a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z"
            />
          </svg>
        </div>
      </div>
    </div>
  </div>
  <delete-alert v-if="isDeleteAlert" @delete-user="deletedPerson" />
  <edit-class v-if="iseditClass" />
</template>

<script>
import DeleteAlert from "../action/DeleteAlert.vue";
import EditClass from "../Edit/EditClass.vue";
export default {
  components: {
    "delete-alert": DeleteAlert,
    "edit-class": EditClass,
  },
  data() {
    return {
      classes: [
        { name: " web a", students: { number: 12 } },
        { name: "web b", students: { number: 22 } },
        { name: "web c", students: { number: 32 } },
      ],
      isRemoveClass: false,
      isDeleteAlert: false,
      iseditClass: false,
    };
  },
  methods: {
    removeClass() {
      this.isRemoveClass = !this.isRemoveClass;
    },
    alertDelete() {
      this.isDeleteAlert = true;
    },
    editClass() {
      this.iseditClass = true;
    },
  },
};
</script>

<style>
#cardAction::before {
  position: absolute;
  content: "";
  height: 13px;
  width: 13px;
  background: #fff;
  left: 60px;
  top: -7px;
  transform: translateX(-50%) rotate(45deg);
}
</style>
