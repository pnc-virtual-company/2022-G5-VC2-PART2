<template>
  <table class="table-auto justify-between w-full rounded shadow p-4">
    ...
    <tbody class="bg-slate-100">
      <tr class="text-center relative p-2 h-16 hover:bg-slate-200 cursor-pointer group" v-for="teacher of peopleList"
        :key="teacher.id">
        <td class="text-center">
          <div class="flex items-cener justify-center">
            <img alt="Vue logo" :src="teacher.profile" class="w-10 h-10 rounded-full" />
          </div>
        </td>
        ...
        <td class="text-left flex justify-between mt-5">
          ...
          <div class="flex absolute justify-end right-2">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
              stroke="currentColor" class="w-5 h-5 mr-4 hidden group-hover:block text-red-500 cursor-pointer">
