    ...
    <nav class="flex w-full p-2 text-white justify-between items-center px-4">
      <ul class="flex items-center">
        <li class="mr-16">
          ...
          <li class="relative" v-if="user.roles != 'STUDENT'">
          ...
          <li class="relative" v-if="user.roles === 'STUDENT'">
            <router-link class="p-3 px-4" exact-active-class="exact-active-link" to="/yourFollowup">Your FollowUp</router-link>
          </li>
      </ul>
      <li
        @click="show = !show"
      ...
      >
          <div class=" inline-flex relative w-fit">
            <router-link to="/notifycation"></router-link>
        ...
        <div class="flex justify-center items-center" @click="isClickShow=!isClickShow">
          ...
          <svg v-if="!isClickShow" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
          ...
          <div id="card" class="bg-[#ffffff] z-50  ease-in-out duration-500 rounded shadow absolute right-2 top-14   text-primary w-40 p-2" v-if="isClickShow">
